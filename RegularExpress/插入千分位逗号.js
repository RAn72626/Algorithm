...
console.log(str.match(regx), str.match(regx1))


// var pattern = /regex/flag, /regex/ 是正则，flag是控制筛选条件，g是global，i是忽略大小写
// //s, 单行模式，此模式下 (.)点号不匹配换行符，在windows中与[^\r\n]等价，并不是刻意匹配任意字符
// //m, 多行模式，此模式下，^和$的含义会被改变，普通的含义是匹配整个字符串的开头和结尾，在多行模式下匹配的就是每一行的行首和行尾。而\A和\Z他们的含义始终是匹配整个字符串的开头和结尾
